                ...
                case "add":
                    ...
                    try {
                        ...
                        const Channel = interaction.options.get('channel').value;
                          ...
                          if (QueryAutoDelete == 1) return interaction.reply({ content: `<#${Channel}> is already an autodelete channel. You cannot set the suggestion channel here.`, ephemeral: true, });

                          const QueryAutoPublish = await AutoPublish.count({ where: { ServerID: interaction.guildId.toString(), ChannelID: Channel }, });
                          if (QueryAutoPublish == 1) return interaction.reply({ content: `<#${Channel}> is already an autopublish channel. You cannot set the suggestion channel here.`, ephemeral: true, });
